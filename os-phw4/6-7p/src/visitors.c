...
void waitForHotelToOpen(int hotel_sock, struct sockaddr_in hotel_addr) {
  char buffer[MAX_MSG_SIZE];

  printf("\nWaiting for the hotel to open\n");
  getStringResponce(hotel_sock, hotel_addr, sizeof(hotel_addr), buffer, true);
...
void visitorsProcess(int hotel_sock, struct sockaddr_in hotel_addr) {
  waitForHotelToOpen(hotel_sock, hotel_addr);

  int visitors_cnt = getIntegerResponceOnMessage(
      hotel_sock, "visitors cnt", hotel_addr, sizeof(hotel_addr), true);
  ...
  while (true) {
    visitors_cnt = getIntegerResponceOnMessage(
        hotel_sock, "visitors cnt", hotel_addr, sizeof(hotel_addr), true);
    if (visitors_cnt == 0) {
      printf("\nAll visitors left the hotel\n");
      sendMessage(hotel_sock, "the end", hotel_addr, sizeof(hotel_addr), true);
      break;
    }

    int waiting_visitors_nums[visitors_cnt];
    int waiting_visitors_days_to_stay[visitors_cnt];
    sendMessage(hotel_sock, "waiting num days", hotel_addr, sizeof(hotel_addr),
                true);
    for (int i = 0; i < visitors_cnt; ++i) {
      waiting_visitors_nums[i] =
          getIntegerResponce(hotel_sock, hotel_addr, sizeof(hotel_addr), true);
      waiting_visitors_days_to_stay[i] =
          getIntegerResponce(hotel_sock, hotel_addr, sizeof(hotel_addr), true);
    ...
    int free_room = getIntegerResponceOnMessage(
        hotel_sock, "free room", hotel_addr, sizeof(hotel_addr), true);
    for (int i = 0; i < visitors_cnt && free_room >= 0;
         ++i, free_room = getIntegerResponceOnMessage(
                  hotel_sock, "free room", hotel_addr, sizeof(hotel_addr),
                  true)) {
      sendMessage(hotel_sock, "occupy room days num", hotel_addr,
                  sizeof(hotel_addr), true);
      sendIntegerMessage(hotel_sock, free_room, hotel_addr, sizeof(hotel_addr),
                         true);
      sendIntegerMessage(hotel_sock, waiting_visitors_days_to_stay[i],
                         hotel_addr, sizeof(hotel_addr), true);
      sendIntegerMessage(hotel_sock, i, hotel_addr, sizeof(hotel_addr), true);
      printf("\nVisitor %d is staying in the room %d for %d days\n", i,
             free_room, waiting_visitors_days_to_stay[i]);
      inrooms_visitors_list[i] = waiting_visitors_days_to_stay[i];
    }

    sendMessage(hotel_sock, "all done", hotel_addr, sizeof(hotel_addr), true);
...
int main(int argc, char *argv[]) {
  ...
  sendMessage(hotel_sock, "visitors", hotel_addr, sizeof(hotel_addr), true);

  visitorsProcess(hotel_sock, hotel_addr);
