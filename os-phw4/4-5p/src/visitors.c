...
void visitorsProcess(int hotel_sock, struct sockaddr_in hotel_addr) {
  ...
  int visitors_cnt = getIntegerResponceOnMessage(
      hotel_sock, "visitors cnt", hotel_addr, sizeof(hotel_addr));
  ...
  while (true) {
    visitors_cnt = getIntegerResponceOnMessage(hotel_sock, "visitors cnt",
                                               hotel_addr, sizeof(hotel_addr));
    if (visitors_cnt == 0) {
      printf("\nAll visitors left the hotel\n");
      sendMessage(hotel_sock, "the end", hotel_addr, sizeof(hotel_addr));
      break;
    }
    ...
    for (int i = 0; i < visitors_cnt; ++i) {
      waiting_visitors_nums[i] =
          getIntegerResponce(hotel_sock, hotel_addr, sizeof(hotel_addr));
    ...
    int free_room = getIntegerResponceOnMessage(hotel_sock, "free room",
                                                hotel_addr, sizeof(hotel_addr));
    ...
    sendMessage(hotel_sock, "all done", hotel_addr, sizeof(hotel_addr));
...
int main(int argc, char *argv[]) {
  ...
  sendMessage(hotel_sock, "visitors", hotel_addr, sizeof(hotel_addr));
